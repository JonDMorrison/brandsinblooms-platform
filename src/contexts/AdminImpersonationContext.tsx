...
import { assertImpersonationSessionResponse, assertImpersonationContext, ImpersonationContext as ImpersonationContextData } from '@/src/lib/database/rpc-types'
...
export function AdminImpersonationProvider({ children }: AdminImpersonationProviderProps) {
  ...
  const startImpersonation = useCallback(async (
    ...
    try {
      ...
      const { data, error: rpcError } = await supabase.rpc('start_admin_impersonation', {
        ...
        impersonated_user_uuid: params.impersonatedUserId || undefined,
        purpose_text: params.purpose || undefined,
        duration_hours: params.durationHours || 2,
        allowed_actions_list: params.allowedActions || undefined,
        ip_addr: undefined, // Will be populated by the function if needed
        user_agent_val: typeof navigator !== 'undefined' ? navigator.userAgent : undefined
      })
      ...
      const sessionData = assertImpersonationSessionResponse(data)
      
      // Validate the session with the server
      const sessionContext = await validateSession(sessionData.session_token)
      if (!sessionContext) {
      ...
      if (typeof window !== 'undefined') {
        localStorage.setItem(IMPERSONATION_TOKEN_KEY, sessionData.session_token)
      }
  ...
  const validateSession = useCallback(async (token: string): Promise<ImpersonationSession | null> => {
    try {
      ...
      const contextData = assertImpersonationContext(data)
      ...
      return {
        ...
        site_custom_domain: contextData.site_custom_domain,
        impersonated_user_id: contextData.impersonated_user_id,
        impersonated_user_email: undefined,
        impersonated_user_name: undefined,
  ...
  const refreshActiveSessions = useCallback(async (): Promise<void> => {
    ...
    try {
      ...
      const { data, error: rpcError } = await supabase.rpc('get_active_impersonation_sessions', {
        admin_user_uuid: undefined, // Get all sessions for monitoring
        site_uuid: undefined,
        limit_count: 100
      })
