...
import { Site, SiteWithMemberships, SiteMembership, SiteMembershipRole } from '@/src/lib/database/types'
...
export async function getSiteWithMemberships(
  ...
  try {
    ...
    const { data, error } = await supabase
      ...
      .select(`
        *,
        memberships:site_memberships!site_id(
          id,
          user_id,
          site_id,
          role,
...
export async function getUserSites(
  ...
  try {
    ...
    const userSites: UserSiteAccess[] = (data || [])
      .filter((membership: any) => membership.site) // Ensure site exists
      .map((membership: any) => ({
        site: membership.site as Site,
        membership: {
          id: membership.id,
