...
export function SiteImpersonationControls({
  ...
  const { currentSession, isImpersonating } = useCurrentImpersonation()
  ...
  const isImpersonatingThisSite = isImpersonating && currentSession?.site_id === siteId
  ...
  const openSiteInNewTab = useCallback(() => {
    ...
    const urlWithToken = `${siteUrl}?admin_impersonation=${encodeURIComponent(currentSession.session_token)}`
    
    window.open(urlWithToken, '_blank')
  }, [currentSession, siteCustomDomain, siteSubdomain])
        ...
        <CardContent>
          <div className="space-y-4">
            ...
            <div className="grid gap-3 text-sm">
              ...
              {currentSession?.impersonated_user_email && (
                <div className="flex justify-between">
                  <span className="text-muted-foreground">Viewing as:</span>
                  <span className="font-medium">
                    {currentSession.impersonated_user_name || currentSession.impersonated_user_email}
                  </span>
                </div>
              )}
              ...
              {currentSession?.purpose && (
                <div className="flex justify-between">
                  ...
                  <span className="font-medium max-w-48 truncate" title={currentSession.purpose}>
                    {currentSession.purpose}
                  </span>
                </div>
              )}
            ...
            {currentSession && new Date(currentSession.expires_at).getTime() - Date.now() < 10 * 60 * 1000 && (
