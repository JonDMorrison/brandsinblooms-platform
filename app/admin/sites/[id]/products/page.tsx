...
import { SiteProductManager } from '@/src/components/admin/SiteProductManager'
...
export async function generateMetadata({ params }: PageProps<{ id: string }>): Promise<Metadata> {
  try {
    ...
    return {
      title: `Product Management - ${site.name} - Admin`,
      description: `Manage products for ${site.name} site`
    }
  } catch (error) {
    return {
      title: 'Error - Admin',
...
export default async function SiteProductsPage({ params }: PageProps<{ id: string }>) {
  const { id } = await params
  let site: any = null
