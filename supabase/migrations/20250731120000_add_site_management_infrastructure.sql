...
-- Alter existing site_metrics table to add individual metric columns
-- The table was created with JSONB metrics column in previous migration
-- We need to migrate to individual columns for site-domains approach
...
ALTER TABLE public.site_metrics 
DROP CONSTRAINT IF EXISTS site_metrics_site_id_metric_date_key;

ALTER TABLE public.site_metrics 
ADD CONSTRAINT site_metrics_site_id_metric_date_key UNIQUE(site_id, metric_date);
